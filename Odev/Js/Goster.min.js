var geriSayim = $('#kalansure');

geriSayim.countdown(new Date($('#kalansure').attr('data-Tarih')), function (event) {
        $(this).html(event.strftime('%D ' + mesajlar.gun + ' %H ' + mesajlar.saat + ' %M ' + mesajlar.dakika + ' %S ' + mesajlar.saniye + ''));
    });
    ...
    function commitBlockList() {
           ...
           $.ajax({
               ...
               success: function (data, status) {
                   ...
                   DosyayiveOdeviKaydet(odevID,selectedFile.name, selectedFile.size, selectedFile.type,azureDosyaAdi, aciklama);
...
function DosyayiveOdeviKaydet(OdevID,dosyaAdi, dosyaBoyut, dosyaTuru, azureDosyaAdi, aciklama) {

    $.ajax({
        type: 'POST',
        dataType: 'json',
        url: '/Ajax/Odev/OdevGonder?OdevID=' + OdevID + '&AzureDosyaAdi=' + azureDosyaAdi + '&Aciklama=' + aciklama + '&DosyaBoyut=' + dosyaBoyut + '&DosyaAdi=' + dosyaAdi + '&DosyaTuru=' + dosyaTuru,
        success: function (msg) {

            $('#tdOdevTeslimDurumu').html('<p class="text-success"><i class="fa fa-check fa-3x"></i> ' + mesajlar.odevteslimedildi + ' <a href="/Dosya/Indir?DosyaID=' + msg.DosyaID + '" class="btn btn-info"><i class="fa fa-download"></i> ' + mesajlar.karsidanyukle + '</a></p>');
