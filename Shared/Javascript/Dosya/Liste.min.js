   ...
   $("#txtDosya").change(function () {
           ...
           $.ajax({
               ...
               success: function (msg) {
                   ...
                   tabloDosyalar.row.add([
                                              ...
                                              '<button class="btn btn-danger btnDosyaSil btn-sm" data-DosyaID="' + msg.DosyaID + '"><i class="fa fa-remove"></i></button> '
    ...
    $('#btnKlasorOlustur').click(function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                html = '<tr class="KlasorSatiri">' +
                '<td class="col-md-1 text-center">' +
                '<label>' +
                '<input type="checkbox" data-OgeID="k_' + res + '" class="colored-primary chkBoxOge">' +
    ...
    $('body').on('click', '.lnkKlasorSecOge', function () {
        var ustKonum = $(this).attr('data-Konum');
        var Konum = $(this).attr('data-KlasorID');
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                $('#spnKlasorSecUstKonum').html(ustKonum);
                $('#spnKlasorSecKonum').html(Konum);
                if (Konum == -1) {
                    ustKonum = -2;
                }
                if (ustKonum != -2) {
                    $('#tableKlasorSecKlasorListesi').prepend('<tr><td><button class="btn btn-link lnkKlasorSecOge" data-KlasorID="' + ustKonum + '" data-Konum="' + Konum + '"><i class="fa fa-arrow-circle-up fa-2x"></i> Bir Üst Konum</button></td></tr>')
                }
                for (var i = 0; i < res.length; i++) {
                    console.log(res[i].KlasorID);
                }
