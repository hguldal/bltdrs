...
$('#dosyaPaylascmbDersID').selectize();
$('#yaziEklecmbDersID').selectize();
    ...
    function AktiviteJsonToHtml(objAktivite) {
       ...
       var html='';
        ...
        html='<div class="widget bordered-top bordered-blue">' +
    ...
    /********* Dosya Son ***********************************************************************/
    /********* Yazı Paylaş **********************************************************************/
    $('#yaziEkleTamam').click(function () {
        var DersID = $("#yaziEklecmbDersID").val();
        var yazi = $("#yaziEkleYazi").summernote('code');

        var dugme = $(this);
        dugme.attr('disable', 'true');

        if (DersID == '') {
            $("#yaziEklecmbDersID").focus();
            return false;
        }

        if ($('#yaziEkleYazi').summernote('isEmpty')) {
            $('#yaziEkleYazi').summernote('focus');
            return false;
        }

        if (yazi.length > 100000) {
            $('#yaziEkleYazi').summernote('focus');
            HataMesaji(mesajlar.odevinaciklamasicokbuyuk);
            return false;
        }

        $('#modalYaziEkle').modal('hide');
        $('#yaziEkleYazi').summernote('reset');

        $.ajax({
            type: 'POST',
            url: '/Ajax/Ders/YaziEkle',
            data: "DersID=" + DersID + "&Yazi=" + encodeURIComponent(yazi),
            success: function (msg) {
                dugme.attr('disable', 'false');
                MesajKutusu(mesajlar.islemtamam, 'Yazı derste başarıyla paylaşıldı');
            },
            error: function (msg) {
                dugme.attr('disable', 'false');
